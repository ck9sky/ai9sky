...
class DALLE_API_PY_Test1(generic.FormView):
    """ This app (dalle_api_py) is more like a "traditional" Django app because Python openai Library is used (instead
        ...
        -------------------------------------------------------------------------------------------------------------------------
        WHY plus_context? I read online about how to "add more" context to a Django template, and/or update context vars
        on-the-fly such that I am able to send back the image_url generated by OpenAI module--maybe could try to use
        ajax (?), but plus_context works. CACHE ERROR (?) ISSUE: Below, after "using" the plus_context var, I SET IT
        BACK TO NULL STRING--WITHOUT THIS, WHEN USER LEAVES THIS VIEW, THEN BROWSES BACK TO IT, THEY WILL NOT SEE THE
        PREVIOUS IMAGE, very odd, as if the Python object representing this view is "re-used" somehow? 11/18/23
        -------------------------------------------------------------------------------------------------------------------------
