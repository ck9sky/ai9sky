...
class DALLE_API_PY_Test1(generic.FormView):
    ...
    def get_context_data(self, **kwargs):
        """ 
            ...
            to avoid odd "caching error" (read discussion in view DALLE_API_PY_Test1)

            but was not needed in chatgpt_api_py(?) .
            11/18/23
        ----------------------------------------------------------------------------------------------------------------
        WHY use plus_context dict? I read online about how to "add more" context to a Django template, and/or update
        context vars 

        on-the-fly such that I am able to send back the image_url generated by OpenAI module--maybe could
        use Django "ajax" view (?), but plus_context works well.
        ----------------------------------------------------------------------------------------------------------------
        DANGER: CACHE ERROR (?) ISSUE: Below, after "using" the plus_context var, I SET IT BACK TO NULL STRING--WITHOUT
        THIS, WHEN USER LEAVES THIS VIEW, THEN BROWSES BACK TO IT, THEY WILL NOT SEE THE PREVIOUS IMAGE, very odd, as
        if the Python object representing this view is "re-used" somehow? 11/18/23
        -- p.s. In a real Django projct that uses a database, I suspect some of the odd characteristics of this view
        could be avoided? cdk
        """
